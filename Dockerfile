...
ENV VIRTUAL_ENV /opt/fusion-datasets-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/fusion-datasets/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/fusion-datasets/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/fusion_datasets/ && touch src/fusion_datasets/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/fusion-datasets/
